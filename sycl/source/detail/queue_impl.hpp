...
public:
  ...
  event submit_direct_with_event(const sycl::detail::ranges_ref_view &ndr,
    ...
    detail::EventImplPtr EventImpl = submit_direct_impl(
        ndr, SubmitInfo, KRInfo, true, CodeLoc, IsTopCodeLoc);
    return createSyclObjFromImpl<event>(EventImpl);
  }

  void submit_direct_without_event(const sycl::detail::ranges_ref_view &ndr,
                                   const detail::v1::SubmissionInfo &SubmitInfo,
                                   const detail::v1::KernelRuntimeInfo &KRInfo,
                                   const detail::code_location &CodeLoc,
                                   bool IsTopCodeLoc) {
    submit_direct_impl(ndr, SubmitInfo, KRInfo, false, CodeLoc,
                       IsTopCodeLoc);
  }
...
protected:
  ...
  detail::EventImplPtr submit_direct_impl(const sycl::detail::ranges_ref_view &ndr,
                                          const v1::SubmissionInfo &SubmitInfo,
